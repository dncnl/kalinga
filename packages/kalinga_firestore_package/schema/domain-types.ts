...
/**
 * Firestore path: households/{householdId}/careRecipients/{careRecipientId}/symptomChecks/{symptomCheckId}
 * One chat-based symptom-checker turn: caregiver's message, a RAG-grounded answer in her own language, an urgency classification, and a Mandarin summary for family/doctor. Never a diagnosis — see conventions.medicalSafety.
 * Client read policy: authorizedCareTeam; client write policy: serverOnly.
 */
export interface SymptomCheckDocument extends AuditFields, SoftDeleteFields, RetentionFields {
  locale: LocaleCode;
  messageText: string;
  answerText: string;
  sources: Record<string, unknown>[];
  urgency: Urgency;
  familySummaryZh: string;
  alertId: string | null;
  askedByUid: string;
}
...
export interface DocumentByPathPattern {
  ...
  "households/{householdId}/careRecipients/{careRecipientId}/symptomChecks/{symptomCheckId}": SymptomCheckDocument;
