...
async function main() {
  ...
  assert.equal(paths.length, 76, 'Kalinga 1.2.0 must retain all 76 document patterns.');
  ...
  assert.equal(new Set(interfaceNames).size, 76, 'Every document pattern must have a unique interface.');
  assert.equal(new Set(documentHelpers).size, 76, 'Every document pattern must have a unique document helper.');
  assert.equal(new Set(collectionHelpers).size, 76, 'Every document pattern must have a unique collection helper.');
  ...
  assert.equal(schema12.data.collectionManifest.length, 76);
